...
DOC_SRC	    = doc/geradtheme.dtx
DOC_PDF	    = doc/geradtheme.pdf

CTAN_CONTENT = README.md $(INS) $(PACKAGE_SRC) $(DOC_SRC) $(DOC_PDF) $(DEMO_SRC) $(DEMO_PDF)

DESTDIR   ?= $(shell kpsewhich -var-value=TEXMFHOME)
INSTALLDIR = $(DESTDIR)/tex/latex/geradtheme
DOC_DIR    = $(DESTDIR)/doc/latex/geradtheme
...
install: $(PACKAGE_STY) $(DOC_PDF)
	@mkdir -p $(INSTALLDIR)
	@cp $(PACKAGE_STY) $(INSTALLDIR)
	@mkdir -p $(DOC_DIR)
	@cp $(DOC_PDF) $(DOC_DIR)

uninstall:
	@rm -f $(addprefix $(INSTALLDIR)/, $(PACKAGE_STY))
	@rmdir $(INSTALLDIR)
	@rm -f $(addprefix $(DOC_DIR)/, $(DOC_PDF))
	@rmdir $(DOC_DIR)
...
ctan: $(CTAN_CONTENT) ctan-version
	@tar --transform "s@\(.*\)@gerad/\1@" -cf gerad-$(shell date +%Y%m%d).tar.gz $(CTAN_CONTENT)

ctan-version:
	@sed -i 's@20[0-9][0-9]/[0-9][0-9]/[0-9][0-9]*@$(shell date +%Y/%m/%d)@' $(PACKAGE_SRC)
